...
The deployed `best-factor` page continues to work from committed static JSON and the `SonChangGi/best-factor` GitHub Actions workflow. The live-data path runs on a 08:43 KST Tue-Sat primary schedule after expected US regular sessions; 12:13/15:43 KST fallback schedules rerun only when the deployed JSON is stale, missing, or broken. Successful live runs commit the regenerated `docs/data/*.json` files before deploying Pages so later source pushes do not redeploy older committed data. A scheduled provider or analysis failure still rejects the candidate and preserves the last-good files, and the run reports failure even when the previous public page is still usable; public availability is checked separately. Reviewed `workflow_dispatch` and controlled runs remain strict for debugging and API correctness.
