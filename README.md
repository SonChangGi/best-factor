...
The only automated deployment target is `SonChangGi/best-factor`. Live-data cron schedules are active again at 08:43 KST Tue-Sat with 12:13/15:43 KST stale-data fallbacks, and reviewed `workflow_dispatch` runs execute the same live yfinance-primary/Yahoo-chart-fallback generation path. Pushes deploy the committed `docs/` artifact only when it reaches the latest completed session; stale committed snapshots fail before deployment. scheduled/manual refreshes sync `docs/data/latest-results.json`, `docs/data/summary.json`, the result-bound public configuration, and the persisted private configuration before Pages deployment so later pushes do not overwrite a fresher artifact with stale JSON or settings.

For scheduled and manual updates, the deployed Pages artifact remains the freshness source of truth. The freshness gate is implemented in `.github/scripts/check_dashboard_freshness.py`: every primary/retry slot skips when the public JSON already represents the same completed U.S. session. The expected session is frozen before collection, using New York close time, DST, exchange holidays and scheduled early closes; collection and publication must both reach that exact date. Default manual dispatches also reuse a current result only when the effective configuration hash and public result binding match; `force_refresh` permits an explicit rebuild and control-bound requests always execute. Provider retries are bounded, stale long-range Yahoo rows trigger a validated completed-session one-day recovery, and failures preserve the last-good public artifact and remain visible as failed runs. Source SHA guards prevent old queued runs from publishing after newer source changes.
