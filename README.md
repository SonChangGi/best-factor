...
For scheduled and manual updates, the deployed Pages artifact remains the freshness source of truth. The freshness gate is implemented in `.github/scripts/check_dashboard_freshness.py`: every primary/retry slot skips when the public JSON already represents the same completed U.S. session. The expected session is frozen before collection, using New York close time, DST, exchange holidays and scheduled early closes; collection and publication must both reach that exact date. Provider retries are bounded, stale long-range Yahoo rows trigger a validated completed-session one-day recovery, and failures preserve the last-good public artifact and remain visible as failed runs. Source SHA guards prevent old queued runs from publishing after newer source changes.
